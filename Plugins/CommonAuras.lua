...
L:RegisterTranslations("frFR", function() return {
	fw_cast = "%s a prot\195\169g\195\169 par Gardien de la peur %s.",
	fw_bar = "%s: FW Cooldown",
...
function BigWigsCommonAuras:BigWigs_RecvSync( sync, rest, nick )
	...
	if self.db.profile.fearward and sync == "BWCAFW" and rest then
		self:TriggerEvent("BigWigs_Message", string.format(L["fw_cast"], nick, rest), "Yellow", not self.db.profile.broadcast, false)
		self:TriggerEvent("BigWigs_StartBar", self, string.format(L["fw_bar"], nick), 30, BS:GetSpellIcon("Fear Ward"), "Yellow")
